...
# @Create At: 2019-06-30 21:32:36
# @Last Modified By: Mindon Gao
# @Last Modified At: 2019-06-30 22:45:21
...
# read data and transfer to numpy type
...
# normalize data through (data - min) / (max - min)
def auto_norm(data):
    min_val = data.min(0)
    max_val = data.max(0)
...
def dating_class_test():
    ho_rotio = 0.1
    dating_data_mat, dating_labels = file2matrix('./kNN/datingTestSet2.txt')
...
# a system can output what person u like
def classify_person():
    ...
    data, label = file2matrix('./kNN/datingTestSet2.txt')
    norm_data, ranges, min_val = auto_norm(data)
    in_arr = np.array([ffMiles, percent_tats, ice_cream])
    classifier_result, class_label = classify(
        (in_arr-min_val)/ranges, norm_data, label, 3)
